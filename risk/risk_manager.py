...
Session circuit breaker:
  ...
  - Log and SMS alert on halt
  - Restart (next day) resets the counter — no DB dependency
  - Paper mode: circuit breaker fires and logs, but does NOT halt (training mode)
"""
...
from config import (
    ...
    CONTRACT_MULTIPLIER, TRADE_GRADE_C, PAPER_TRADING, INSTRUMENT
)
...
class SizingResult:
    contracts:          int   = 0
    cost_per_contract:  float = 0.0   # $ per contract (premium × 100)
    total_cost:         float = 0.0   # Total $ spent
    max_loss:           float = 0.0   # = total_cost (defined-risk debit)
    grade:              str   = "B"
...
class RiskManager:
    ...
    def compute_size(self,
        ...
        # Halve butterfly size in moderate VIX
        if is_butterfly and butterfly_half_size:
            grade_mult = grade_mult * 0.5

        # Contracts = floor(risk × grade_mult / cost_per_contract)
    ...
    def record_win(self):
        """Call when a trade closes at a profit (does NOT reset loss counter)."""
        logger.info(
    ...
    def _fire_circuit_breaker(self):
        """Halt trading for the remainder of the session."""
        ...
        logger.warning(
            f"   Review trades: python query.py"
        )
        logger.warning(
            f"   Restart tomorrow: sudo systemctl start optionsbot"
        )
        if not self._paper_trading:
            # Stop the service — operator must restart manually
    ...
    def reset_session(self):
        """Call at start of each RTH session."""
        self._session_losses = 0
        self._session_halted = False
