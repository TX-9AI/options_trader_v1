...
Schema matches options-specific fields: strike, expiry, premium, butterfly legs.
"""
...
class TradeLogger:
    ...
    def get_session_losses(self) -> int:
        """Count closed losses since today's RTH open."""
    ...
    def today_summary(self) -> dict:
        """Summary stats for today's session."""
        today = now_utc().strftime("%Y-%m-%d")
        with self._connect() as conn:
            rows = conn.execute("""
