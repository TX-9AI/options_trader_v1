#!/bin/bash
# =============================================================================
# setup_ec2.sh — options_trader v1.0 EC2 Setup
# QQQ 0DTE | TastyTrade OAuth | Telegram alerts
# Files SCP'd to ~/options-trader-deploy/ by install.bat or cloned via install.sh
# =============================================================================
...
echo -e "${BOLD}${CYAN}║     QQQ 0DTE  |  TastyTrade  |  Telegram           ║${RESET}"
echo -e "${BOLD}${CYAN}╚══════════════════════════════════════════════════════╝${RESET}"
echo ""
echo "  Have ready:"
...
print_ok "Instrument: QQQ | Risk: \$${RISK_USD}/trade | Mode: $([ "$PAPER_TRADING" = "True" ] && echo "PAPER" || echo "LIVE")"

# ─── STEP 2: TASTYTRADE CREDENTIALS ─────────────────────────────────────────
print_step "2/7" "TastyTrade OAuth Credentials"
echo ""
...
# ─── STEP 3: TELEGRAM ────────────────────────────────────────────────────────
print_step "3/7" "Telegram Alerts"
echo ""
while true; do
...
print_step "5/7" "Installing bot files"
mkdir -p "$INSTALL_DIR"
rsync -a \
    --exclude='.git' \
    --exclude='*.pem' \
...
print_step "6/7" "Python environment"
python3 -m venv "$VENV"
...
print_step "7/7" "Configuring systemd service"

sudo tee /etc/systemd/system/${SERVICE_NAME}.service > /dev/null << SVCEOF
[Unit]
Description=options_trader v${VERSION} — QQQ 0DTE | Vertigo Capital
...
if [ "$STATUS" = "active" ]; then
    ...
    echo -e "${BOLD}${CYAN}╚══════════════════════════════════════════════════════╝${RESET}"
    echo ""
    echo -e "  Instrument:  QQQ 0DTE (TastyTrade)"
