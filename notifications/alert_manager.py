"""
notifications/alert_manager.py — SMS alerts via Twilio.
Ported from crypto_trader with options-specific message formatting.
"""
...
class AlertManager:
    def __init__(self):
        try:
            ...
            logger.warning(f"SMS not available: {e}")
            self._sms = None
            self._enabled = False

    def _send(self, msg: str):
        if self._enabled and self._sms:
            try:
                self._sms.send(msg)
            except Exception as e:
